...
describe('feed-settings: 設定 CRUD 結合テスト', () => {
  let env: TestEnv
  let token: string
  ...
  beforeAll(async () => {
    ...
    // 元の設定を保存しておく
    const res = await feedClient.get<SettingsResponse>('/settings')
    if (res.ok) {
  ...
  describe('PUT /settings', () => {
    it('webhook_url と webhook_type を更新できる', async () => {
      const updateRes = await feedClient.put<SettingsResponse>('/settings', {
    ...
    it('更新後に GET で更新後の値を確認できる', async () => {
      // 先に更新する
      await feedClient.put<SettingsResponse>('/settings', {
        webhook_url: 'https://hooks.slack.com/test-updated',
        webhook_type: 'slack',
      })

      // GET で確認
      const getRes = await feedClient.get<SettingsResponse>('/settings')
