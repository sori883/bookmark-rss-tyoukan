...
| **cli**          | `cli login` でOAuthフロー実行 → JWTをローカル保存 → API Gateway経由でfeed/notificationへのリクエストにJWT付与                |
| **mcp-server**   | 起動時にJWTを設定（環境変数 or config）→ API Gateway経由でfeed/notificationへのリクエストにJWT付与                           |
...
| 言語             | 対象                            | 理由                                                    |
| -------------- | ----------------------------- | ----------------------------------------------------- |
| **TypeScript** | auth, feed, notification, web | DOM解析(readability)はJS/TSが最適。フロントとバックエンドでエコシステム共有      |
...
sequenceDiagram
    ...
    participant feed
    participant client as web/cli/mcp
...
graph LR
    web -->|REST| feed
    mcp[mcp-server] -->|REST| feed
    cli -->|REST| feed
    web -->|REST| notification
    mcp -->|REST| notification
...
クライアント(web/cli/mcp)およびcron・aiから直接アクセスされる。フィード管理、記事データの一元管理、ユーザ設定管理を担当。
...
クライアント(web/cli/mcp)およびaiサービスから直接アクセスされる。
